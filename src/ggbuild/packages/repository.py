...
from poetry.core.packages import (
    ...
    vcs_dependency as poetry_vcsdep,
)
...
from poetry.repositories import (
    repository as poetry_repo,
    repository_pool as poetry_pool,
)
...
class Provider(poetry_provider.Provider):
    ...
    def _search_for_vcs(
        ...
        python = importlib.import_module("ggbuild.packages.python")
        ...
        return cast("poetry_pkg.Package", package)
...
def extra_requirements(
    ...
    try:
        ...
        dep_group.__dict__["_dependencies"] = orig_reqs


def set_build_requirements(
    pkg: poetry_pkg.Package, reqs: list[poetry_dep.Dependency]
) -> None:
    pkg.__dict__["build_requires"] = list(reqs)
