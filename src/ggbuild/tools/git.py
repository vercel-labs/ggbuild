...
from ggbuild import exceptions
...
class Git(core_git.Git):
    ...
    @classmethod
    def initialize(cls, work_dir: pathlib.Path) -> Git:
        """Initialize and return a repository with a working tree."""
        cmd("git", "init", "--quiet", "--", work_dir)
        return cls(work_dir)
        ...
        try:
            ...
            if when_exit_code is not None:
                ...
                if e.returncode not in codes:
                    raise exceptions.GGBuildRuntimeError.create(
            ...
            try:
                ...
                err = exceptions.GGBuildRuntimeError.create(
    ...
    def work_tree(self) -> pathlib.Path:
        ...
        if work_tree is None:
            raise RuntimeError("bare Git repository has no work tree")
        return work_tree
...
class GitClone(Git):
    ...
    def __init__(self, repo_url: str, work_dir: pathlib.Path) -> None:
        ...
        self.run_or(
            ...
            repo_url,
            when_exit_code=2,
            default=lambda _: self.run("remote", "add", "origin", repo_url),
        )
