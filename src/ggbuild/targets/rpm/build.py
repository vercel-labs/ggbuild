...
from ggbuild import packages as mpkg, targets, tools


class RPMTarget(Protocol):
    distro: dict[str, str]

    def install_build_deps(self, build: Build, spec: str) -> None: ...


class Build(targets.Build):
    @property
    def rpm_target(self) -> RPMTarget:
        return cast("RPMTarget", self._target)
    ...
    def get_path(
        ...
        *,
        ...
        if relative_to == "buildroot":
            return pathlib.Path("..") / path
        if relative_to in {"pkgsource", "pkgbuild", "helpers"}:
            return pathlib.Path("..") / ".." / path
    ...
    def _common_package_spec(self) -> str:
        if not self._bin_shims:
            return ""
        return textwrap.dedent(
            ...
            files=self._get_common_files(),
        )

    def _meta_package_specs(self, root_version: str) -> list[str]:
        package_template = textwrap.dedent(
            """\
            %package -n {name}
            Summary: {description}
            Group: {group}
        ...
        specs = [
            package_template.format(
                ...
                group=self._root_pkg.group,
                dependencies="\n".join(
                    f"Requires: {name}{' ' if version else ''}{version or ''}"
        ...
        specs.extend(
            package_template.format(
                ...
                description=(
                    ...
                    f"use {self._root_pkg.name_slot} instead"
                ),
                license=self._root_pkg.license,
                url=self._root_pkg.url,
                group=self._root_pkg.group,
                dependencies=(
                    f"Requires: {self._root_pkg.name_slot} = {root_version}"
                ),
            )
    ...
    def _write_spec(self) -> None:
        ...
        rev = f"{self._revision}{self._subdist or ''}"
        root_v = self._format_version()
        root_version = f"{root_v}-{rev}%{{?dist}}"
        meta_pkg_specs = self._meta_package_specs(root_version)
        ...
        """
            ...
            unpack_script=self._write_script("unpack", relative_to="buildroot"),
            ...
            common_pkg=self._common_package_spec(),
            meta_pkgs="\n\n".join(meta_pkg_specs),
            dbgsym_pkg="%debug_package" if self._build_dbgsym else "",
        )

        spec_root = self.get_spec_root(relative_to="fsroot")
        pathlib.Path(spec_root / f"{self._root_pkg.name_slot}.spec").write_text(
    ...
    def _get_changelog(self) -> str:
        ...
        """
            ...
            date=datetime.datetime.now(datetime.UTC).strftime("%a %b %d %Y"),
    ...
    def _get_build_reqs_spec(self) -> str:
        ...
        lines = [f"BuildRequires: {pkg.system_name}" for pkg in deps]
    ...
    def _get_runtime_reqs_spec(self, extrareqs: dict[str, set[str]]) -> str:
        ...
        lines = [f"Requires: {pkg.system_name}" for pkg in deps]
        ...
        for category, reqs in extrareqs.items():
            rpm_category = categorymap[category]
            lines.append(f"Requires({rpm_category}): {' '.join(reqs)}")

        return "\n".join(lines)

    def _get_conflict_spec(self, conflicts: list[str]) -> str:

        lines = [f"Conflicts: {conflict}" for conflict in conflicts]
    ...
    def _get_package_install_script(self, pkg: mpkg.BasePackage) -> str:
        ...
        ignored_dep_text = self.render_package_script(pkg, "ignored_dependency")
    ...
    def _get_install_extras(self) -> str:
        ...
        for pkg in self._installable:
            ...
            symlinks.extend(
                (cmd, f"{cmd.name}{pkg.slot_suffix}")
        ...
        if symlinks:
            ...
            for src, tgt in symlinks:
                lines.append(
                    f'ln -sf "{src}" "${{RPM_BUILD_ROOT}}/%{{_bindir}}/{tgt}"'
                )
        ...
        if self._bin_shims:
            ...
            for shim_path, data in self._bin_shims.items():
                ...
                pathlib.Path(inst_path).write_text(data, encoding="utf-8")
                pathlib.Path(inst_path).chmod(
                    stat.S_IRWXU
                    | stat.S_IRGRP
                    | stat.S_IXGRP
                    | stat.S_IROTH
    ...
    def _get_common_files(self) -> str:
        if self._bin_shims:
            return "\n".join(f"%{{_bindir}}/{path}" for path in self._bin_shims)
        return ""

    def _rpmbuild(self) -> None:
        self.rpm_target.install_build_deps(
    ...
    def package(self) -> None:
        ...
        distro = self.rpm_target.distro["codename"]
        rev = f"{self._revision}{self._subdist or ''}"
        root_v = self._format_version()
