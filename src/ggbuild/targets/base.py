...
from lograil import stage, update_status

from ggbuild import tools
from ggbuild.packages import base as mpkg_base, sources as mpkg_sources

from . import _helpers as helpers_pkg, package as tgt_pkg
...
class BuildRequest(NamedTuple):
    ...
    deps: Sequence[mpkg_base.BasePackage] = ()
    build_deps: Sequence[mpkg_base.BasePackage] = ()
    ...
    jobs: int = 1
    compression: Sequence[str] = ("zstd",)
    prebuilt_packages: Sequence[str] = ()
    node_staging: pathlib.Path | None = None
    bundle_install_subdir: pathlib.Path | None = None
    node_cache: Mapping[str, str] | None = None
...
class _TargetIdentity:
    ...
    def get_builder_instance(self, request: BuildRequest) -> Build:
        return self.get_builder()(cast("Target", self), request)
...
class _TargetPackaging(_TargetIdentity):
...
class LinuxTarget(PosixTarget):
    ...
    def service_scripts_for_package(
        ...
        if self.has_capability("systemd"):
            ...
            if systemd_path is None:
                raise RuntimeError(
                    "systemd-enabled target does not define a units path"
                )
    ...
    def supports_lto(self) -> bool:
        ...
        m = re.match(r"^gcc.*?(\d+(?:\.\d+)+)", gcc_ver, re.MULTILINE)
        if not m:
    ...
    def sh_get_package_ld_env(
        ...
        if libdir is not None:
            ...
            return {
                "LD_LIBRARY_PATH": (
                    shlex.quote(str(pkg_lib_path))
                    if pkg_lib_path.is_absolute()
                    else f"{wd}/{shlex.quote(str(pkg_lib_path))}"
                ),
            }
        return {}
    ...
    def get_shlib_relpath_run_time_ldflags(
        ...
        rpath = f"$ORIGIN/{shlex.quote(path)}" if path else "$ORIGIN"
        ...
        flag_digest = hashlib.md5(
            flags.encode(), usedforsecurity=False
        ).hexdigest()
        flag_file_name = f"ld-{flag_digest}"
...
class FHSTarget(PosixTarget):
    ...
    def get_install_path(
        ...
        paths = {
            ...
            "data": root / "usr" / "share" / root_subdir,
            "legal": root / "usr" / "share" / "doc" / root_subdir / "licenses",
            "doc": root / "usr" / "share" / "doc" / root_subdir,
            "info": root / "usr" / "share" / "info",
            "man": root / "usr" / "share" / "man",
            "bin": prefix / "bin",
            "lib": prefix / "lib",
            "include": root / "usr" / "include" / root_subdir,
            "localstate": root / "var",
            "runstate": root / "run",
        }
        try:
...
class _BuildState:
    def __init__(
        ...
        self._keepwork = request.keepwork
        ...
        self._build_dbgsym = request.build_dbgsym
        ...
        self._prebuilt_packages = frozenset(request.prebuilt_packages)
        self._node_staging = request.node_staging
        self._bundle_install_subdir = request.bundle_install_subdir
        self._node_cache = dict(request.node_cache or {})
        if self._jobs == 0:
        ...
        self._sccache_configured = False
        self._sccache_started = False
        self._tarballs: dict[
    ...
    @property
    def is_dbgsym_build(self) -> bool:
        return self._build_dbgsym

    @property
    def is_node_build(self) -> bool:
        return self._node_staging is not None
    ...
    def get_package(self, name: str) -> mpkg_base.BasePackage:
        for candidate in itertools.chain(self._deps, self._build_deps):
            if candidate.name == name:
                if isinstance(candidate, mpkg_base.AliasPackage):
                    aliased = next(iter(candidate.all_requires))
                    return self.get_package(aliased.name)
                return candidate
    ...
    def get_packages(
        ...
        *,
        ...
        build = cast("Build", self)
        return _BuildLifecycle.resolve_packages(
            build, names, recursive=recursive, bundled_only=bundled_only
        )


class _BuildLifecycle(_BuildState):
        ...
        *,
        ...
        return sorted(
            (
                pkg
                for pkg in packages
                if isinstance(pkg, mpkg_base.BundledPackage)
            ),
            key=lambda item: item.unique_name,
        )
    ...
    def run(self) -> None:
        ...
        try:
            ...
            update_status(
                process="packaging",
                subject=f"{self._root_pkg} for {self.target.triple}",
            )
            with stage(
                "package/prepare",
                process="packaging",
    ...
    def _find_sccache(self) -> None:
        requested = os.environ.get("SCCACHE")
        found = requested or shutil.which("sccache")
        if found is None or not pathlib.Path(found).is_file():
            self._enable_sccache = False
            detail = (
                f"SCCACHE points to unavailable executable {requested!r}"
                if requested
                else "sccache was not found in PATH"
            )
            self._io.write_line(
                f"<warning>sccache was requested but {detail}; "
                "continuing without compiler caching.</warning>"
            )
            return
        if not os.access(found, os.X_OK):
            self._enable_sccache = False
            self._io.write_line(
                f"<warning>sccache executable {found!r} is not executable; "
                "continuing without compiler caching.</warning>"
            )
            return
        self._real_sccache_path = pathlib.Path(found).resolve()
        self._system_tools["sccache"] = str(self._real_sccache_path)

    def prepare(self) -> None:
        self.define_tools()

    def _sccache_environment(self) -> dict[str, str]:
        if not self.is_sccache_enabled:
            return {}
        digest = hashlib.sha256(str(self._droot.resolve()).encode()).hexdigest()
        environment = {
            "SCCACHE": str(self._real_sccache_path),
        }
        if sys.platform.startswith("linux"):
            environment["SCCACHE_SERVER_UDS"] = (
                f"\\x00ggbuild-sccache-{digest[:20]}"
            )
        else:
            port = 20_000 + int(digest[:8], 16) % 40_000
            environment["SCCACHE_SERVER_PORT"] = str(port)
        return environment

    def _start_sccache(self) -> None:
        if not self.is_sccache_enabled or self._sccache_started:
            return
        if not self._sccache_configured:
            raise RuntimeError("sccache tools were not configured")
        executable = self._real_sccache_path
        if executable is None:
            raise RuntimeError("sccache executable is not configured")
        subprocess.run(  # ruff: ignore[subprocess-without-shell-equals-true] - argv only
            [executable, "--start-server"],
            check=True,
            env=os.environ | self._sccache_environment(),
            shell=False,
        )
        self._sccache_started = True

    def _stop_sccache(self) -> None:
        if not self._sccache_started:
            return
        executable = self._real_sccache_path
        if executable is None:
            raise RuntimeError("sccache executable is not configured")
        stats = subprocess.run(  # ruff: ignore[subprocess-without-shell-equals-true] - argv only
        ...
        if stats.returncode:
            self._io.write_line(
                "<warning>failed to report isolated sccache "
                "statistics.</warning>"
            )
        result = subprocess.run(  # ruff: ignore[subprocess-without-shell-equals-true] - argv only
        ...
        if result.returncode:
            self._io.write_line(
                "<warning>failed to stop the isolated sccache server.</warning>"
            )
        self._sccache_started = False
    ...
    def prepare_packaging(self) -> None:
        cast("Build", self).get_intermediate_output_dir(
            relative_to="fsroot"
    ...
    def shrinkwrap(self) -> None:
        build = cast("Build", self)
        intermediates = build.get_intermediate_output_dir(relative_to="fsroot")
        self._outputroot.mkdir(parents=True, exist_ok=True)
        for artifact in intermediates.iterdir():
            if not artifact.is_file():
                continue
            output_name = (
                f"{self.artifact_fqname}.metadata.json"
                if artifact.name == "build-metadata.json"
                else artifact.name
            )
            shutil.copy2(artifact, self._outputroot / output_name)

    @property
    def artifact_fqname(self) -> str:
        pkg = self._root_pkg
        pkg_name = (
            str(pkg.name) if pkg.version_includes_slot() else pkg.name_slot
        )
        pkg_ver = mpkg_base.pep440_to_semver(pkg.version)
        return f"{pkg_name}__{pkg_ver}__{self.target.ident}"


class _BuildPaths(_BuildLifecycle):
    @property
    def _as_build(self) -> Build:
        return cast("Build", self)
    ...
    def get_bundle_install_root(self) -> pathlib.Path:
        return self._target.get_bundle_install_root(self._as_build)
    ...
    def get_bundle_install_subdir(self) -> pathlib.Path:
        ...
        if self._bundle_install_subdir is not None:
            return self._bundle_install_subdir
        return self._target.get_bundle_install_subdir(self._as_build)
    ...
    def get_bundle_install_prefix(self) -> pathlib.Path:
        ...
        return self.get_bundle_install_root() / self.get_bundle_install_subdir()
    ...
    def get_install_prefix(self, pkg: mpkg_base.BasePackage) -> pathlib.Path:
        ...
        if pkg != self._root_pkg:
            prefix /= self._root_pkg.get_dep_install_subdir(self._as_build, pkg)
        return prefix
    ...
    def get_bundle_install_path(
        ...
        prefix = self.get_bundle_install_prefix()
        subdir = self.root_package.get_root_install_subdir(self._as_build)
        return self._target.get_install_path(
            self._as_build, root, subdir, prefix, aspect
        )
    ...
    def get_install_path(
        ...
        if prefix != self.get_bundle_install_prefix():
            ...
            return self._get_prefixed_install_path(root, subdir, prefix, aspect)
        return self._target.get_install_path(
            self._as_build, root, subdir, prefix, aspect
        )
    ...
    def get_rel_install_path(
        ...
        """Return an aspect install path relative to the install prefix."""
        prefix = self.get_install_prefix(pkg)
    ...
    def _get_prefixed_install_path(
        ...
        return self._target.get_install_path(
            self._as_build, root, root_subdir, prefix, aspect
        )
...
class _BuildShell(_BuildPaths):
    ...
    def sh_format_args(
        ...
        *,
        ...
        args_parts = []
        for raw_arg, raw_value in args.items():
            if raw_value is None:
                args_parts.append(shlex.quote(raw_arg))
            else:
                value = str(raw_value)
                value = (
                    shlex.quote(value)
                ...
                sep = "=" if raw_arg.startswith("--") or force_args_eq else " "
                quoted_arg = shlex.quote(raw_arg)
                args_parts.append(f"{quoted_arg}{sep}{value}")
    ...
    def write_helper(
        ...
        helper_path = pathlib.Path(helpers_abs / name)

        content = f"{text}\n"
        content_bytes = content.encode("utf-8")
        if (
            not helper_path.exists()
            or helper_path.read_bytes() != content_bytes
        ):
            helper_path.write_bytes(content_bytes)
        if stat.S_IMODE(helper_path.stat().st_mode) != 0o755:
    ...
    def sh_write_bash_helper(
        ...
        *,
        ...
        helper = self.sh_write_helper(name, script, relative_to=relative_to)
        return f'"$BASH" {helper}' if with_interpreter else helper


class _BuildSources(_BuildShell):
    @property
    def _as_build(self) -> Build:
        return cast("Build", self)
    ...
    def get_tarball_tpl(self, package: mpkg_base.BasePackage) -> str:
        rp = self._root_pkg
        package_version = f"{rp.name_slot}_{rp.version.text}"
        return f"{package_version}.orig-{package.name}{{part}}.tar{{comp}}"
    ...
    def get_tool_list(self) -> list[str]:
        tools = ["trim-install.py", "copy-tree.py"]
        if self.is_sccache_enabled:
            tools.append("sccache-wrapper.sh")
        return tools

    def get_su_script(self, script: str, user: str) -> str:
        return self.target.get_su_script(self._as_build, script, user)

    def prepare_tools(self) -> None:
        for pkg in self._bundled:
            bundled_tools = pkg.get_build_tools(self._as_build)
            if bundled_tools:
        ...
        if mod_file is None:
            raise RuntimeError(f"module {self.__module__} has no source file")
        ...
        for helper in self.get_tool_list():
            for source_dir in source_dirs:
                if (source_dir / helper).exists():
                    ...
                    destination = helpers_target_dir / helper
                    if (
    ...
    def _configure_sccache(self) -> None:
        ...
        wrapper = self._common_tools.get("sccache-wrapper")
        if wrapper is None:
            raise RuntimeError("sccache wrapper helper is missing")
        wrapper_path = self.get_path(wrapper, relative_to="fsroot")
        candidate_names = {
            "c++",
            "c99",
            "cc",
        ...
        names = {"sccache"}
        names.update(name for name in candidate_names if shutil.which(name))
        for compiler_group in (("gcc", "g++"), ("clang", "clang++")):
            compiler = shutil.which(compiler_group[0])
            if compiler is None:
                continue
            result = subprocess.run(  # ruff: ignore[subprocess-without-shell-equals-true] - argv only
            ...
            if result.returncode == 0 and machine:
                qualified = (f"{machine}-{name}" for name in compiler_group)
                names.update(name for name in qualified if shutil.which(name))
        try:
            for name in sorted(names):
                link = link_dir / name
                link.unlink(missing_ok=True)
                link.symlink_to(wrapper_path)
        except OSError as error:
            self._enable_sccache = False
            self._io.write_line(
                f"<warning>could not create sccache compiler wrappers: "
                f"{error}; continuing without compiler caching.</warning>"
            )
            return
        self._sccache_configured = True

    def get_sccache_build_env(self, *, make: bool = False) -> dict[str, str]:
        if not self.is_sccache_enabled or not self._sccache_configured:
            return {}
        link_dir = self.get_sccache_link_dir(relative_to="fsroot")
        if make:
            path = os.pathsep.join(
                ...
                if "ccache" not in entry
            )

            def identity(value: str) -> str:
                return value

            quote: Callable[[str], str] = identity
        else:
            path = '"$PATH"'
            quote = shlex.quote
        return {
            "CCACHE_DISABLE": "true",
            "PATH": f"{quote(str(link_dir))}:{path}",
            "RUSTC_WRAPPER": quote(str(link_dir / "sccache")),
            **{
                key: quote(value)
                for key, value in self._sccache_environment().items()
            },
        }
    ...
    def prepare_tarballs(self) -> None:
        ...
        for pkg in self._bundled:
            if str(pkg.name) in self._prebuilt_packages:
                continue
            ...
            for src in pkg.get_sources():
                if src.path:
                    ...
                    part = "" if counter == 0 else f"-{counter}"
                    counter += 1
                source_map[src] = part

            for source, part in source_map.items():
                existing = sorted(
                    tarball_root.glob(tarball_tpl.format(part=part, comp="*"))
                )
                if len(existing) == 1:
                    tarball = existing[0]
                else:
    ...
    def unpack_sources(self) -> None:
        if isinstance(self._root_pkg, mpkg_base.PrePackagedPackage):
            if len(self._tarballs) != 1:
                raise RuntimeError("expected one pre-packaged source")
            tarballs = next(iter(self._tarballs.values()))
            if len(tarballs) != 1:
                raise RuntimeError("expected one pre-packaged tarball")
        ...
        for pkg in self._bundled:
            ...
            for pkgname, patches in pkg.get_patches().items():
                for patch_label, patch in patches:
                    ...
                    patch_suffix = f"--{patch_label}" if patch_label else ""

                    filename = f"{i:04d}-{pkgname}{patch_suffix}.patch"

                    pathlib.Path(patches_dir / filename).write_text(
                        fixed_patch, encoding="utf-8"
                    )

                    series.append((pkgname, filename))
                    i += 1

        with pathlib.Path(patches_dir / "series").open(
            "w", encoding="utf-8"
        ) as f:
    ...
    def get_extra_system_requirements(self) -> dict[str, set[str]]:
        ...
        for pkg in self._installable:
            reqs = pkg.get_extra_system_requirements(self._as_build)
    ...
    def get_service_scripts(self) -> dict[pathlib.Path, str]:
        ...
        for pkg in self._installable:
            pkg_scripts = pkg.get_service_scripts(self._as_build)
    ...
    def get_script(
        ...
        *,
        ...
        for pkg in packages:
            for stg in stages:
                script = _BuildScripts.render_package_script(
                    self._as_build, pkg, stg, relative_to=relative_to
                )
                if script.strip():
...
class _BuildScripts(_BuildSources):
    @property
    def _as_build(self) -> Build:
        return cast("Build", self)
    ...
    def _get_global_after_install_script(self) -> str:
        ...
        if script:
            script = (
                'if [ -z "${_EDGEDB_INSTALL_SKIP_BOOTSTRAP}" ]; then\n'
                f"{script}\nfi"
            )
    ...
    def get_ld_env(
        ...
        keys = self.target.get_ld_env_keys(self._as_build)
        ...
        for pkg in deps:
            ...
            pkg_ld_env = self.target.sh_get_package_ld_env(
                self._as_build, pkg, wd
            )
        ...
        for k, vv in env.items():
            self._as_build.sh_prepend_quoted_paths(env_args, k, vv + extra_vals)
...
class _BuildDependencies(_BuildScripts):
    @property
    def _as_build(self) -> Build:
        return cast("Build", self)
    ...
    def sh_append_pkgconfig_paths(
        ...
        for dependency in pkgs:
            rel_path = self.sh_get_bundled_install_path(
                dependency, relative_to=relative_to, wd=wd
            )

            prefix = self.get_install_prefix(dependency)
            rel_lib_dir = self.get_install_path(dependency, "lib").relative_to(
                prefix
            )

            self._as_build.sh_prepend_quoted_paths(
                args,
                "PKG_CONFIG_PATH",
        ...
        if self._node_staging is not None:
            args["PKG_CONFIG_SYSROOT_DIR"] = shlex.quote(
                str(self._node_staging)
            )
    ...
    def sh_get_bundled_pkg_lib_path(
        ...
        if wd is None:
            wd = "$(pwd -P)"
        if not self.is_bundled(pkg):
            raise ValueError(f"{pkg.name} is not a bundled package")
        ...
        shlib_path = pkg.get_install_path(self._as_build, "lib")
        if shlib_path is not None:
            lib_path = root_path / shlib_path.relative_to("/")
            if lib_path.is_absolute():
                return shlex.quote(str(lib_path))
    ...
    def sh_get_bundled_pkg_include_path(
        ...
        rel_inc_path = pkg.get_install_path(self._as_build, "include")
        if rel_inc_path is not None:
            ...
            if inc_path.is_absolute():
                return shlex.quote(str(inc_path))
    ...
    def sh_get_bundled_pkg_cppflags(
        self,
        pkg: mpkg_base.BasePackage,
        relative_to: Location = "pkgbuild",
        wd: str | None = None,
    ...
    def sh_get_bundled_pkgs_cppflags(
        self,
        deps: Iterable[mpkg_base.BasePackage],
        relative_to: Location = "pkgbuild",
        relative_to_package: mpkg_base.BasePackage | None = None,
        ...
        for pkg in deps:
            if self.is_bundled(pkg):
                flags.extend(
                    self.sh_get_bundled_pkg_cppflags(
                        pkg,
                        relative_to=relative_to,
                        relative_to_package=relative_to_package,
                        wd=wd,
...
class Build(_BuildDependencies):
    def sh_append_global_flags(
        ...
        for key, value in self.get_sccache_build_env().items():
            conf_args[key] = f"!{value}"
        if global_cflags:
        ...
        *,
        ...
        if existing_flags and how != "replace":
            if not isinstance(existing_flags, str):
                raise TypeError(f"{key} must contain string flags")
