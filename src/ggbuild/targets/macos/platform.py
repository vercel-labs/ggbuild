...
from ggbuild.targets import base as targets, generic, package as tgt_pkg
    ...
    def get_script(
        ...
        *,
        ...
        if group:
            ...
            groupadd_cmds.extend((
                self._get_dscl_cmd(groupname, key="Password", value="*"),
                self._get_dscl_cmd(
                    groupname,
                    key="PrimaryGroupID",
                    value=f"!$(($({last_gid}) + 1))",
        ...
        if shell:
            useradd_cmds.append(
                self._get_dscl_cmd(username, key="UserShell", value="/bin/bash")
            )
        else:
            useradd_cmds.append(
                self._get_dscl_cmd(
...
_sys_shlibs = [rf"{_lib_base}/{lib}\.dylib" for lib in _libs] + [
    rf"{_frameworks_base}/{fw}\.framework/Versions/A/{fw}" for fw in _frameworks
]
...
class _MacOSTargetBinary(generic.GenericTarget):
    def __init__(self, arch: str) -> None:
        super().__init__(arch, libc="libSystem")

    @property
    def name(self) -> str:
        return "Generic macOS"
    ...
    def prepare(self) -> None:
        if not shutil.which("brew"):
            logger.info(
                "No Homebrew detected; skipping build-tool auto-installation"
            )
            return

        brew_inst = (
            'for tool do if ! brew ls --versions "$tool"; then '
            'env HOMEBREW_NO_AUTO_UPDATE=1 brew install "$tool"; fi; done'
        )
        tools.cmd(
            "lockf",
            pathlib.Path(tempfile.gettempdir()) / "ggbuild-homebrew.lock",
            "/bin/sh",
            "-c",
            brew_inst,
            "--",
            *self._get_necessary_host_tools(),
        )
    ...
    def get_shlib_refs(
        ...
        *,
        ...
        for output_line in output.strip().split("\n"):
            ...
            if not line:
                continue
            state, value = self._parse_otool_line(state, line)
            if value is None:
                continue
            if state == "dylib_value":
                dylib = pathlib.Path(value)
                if dylib.parts[0] == "@rpath" and resolve:
                    dylib = pathlib.Path(*dylib.parts[1:])
                shlibs.add(dylib)
            else:
                rpath = pathlib.Path(value)
                if value.startswith("@loader_path") and resolve:
                    rpath = (
                        pathlib.Path("/")
                        / install_path.parent
    ...
    @staticmethod
    def _parse_otool_line(state: str, line: str) -> tuple[str, str | None]:
        if re.match(r"^Load command (\d+)\s*$", line, re.IGNORECASE):
            return "load_cmd", None
        if re.match(r"^Section$", line, re.IGNORECASE):
            return "skip", None
        if state == "load_cmd":
            commands = {
                "cmd LC_LOAD_DYLIB": "lc_load_dylib",
                "cmd LC_RPATH": "lc_rpath",
            }
            return commands.get(line, state), None
        patterns = {
            "lc_load_dylib": ("dylib_value", r"^name\s+([^(]+).*$"),
            "lc_rpath": ("rpath_value", r"^path\s+([^(]+).*$"),
        }
        if state not in patterns:
            return state, None
        value_state, pattern = patterns[state]
        match = re.match(pattern, line)
        return (value_state, match.group(1).strip()) if match else (state, None)


class MacOSTarget(_MacOSTargetBinary):
...
class MacOSNativePackageTarget(MacOSTarget):
    ...
    def service_scripts_for_package(
        ...
        if launchd_path is None:
            raise RuntimeError("system daemon resource path is unavailable")
...
def get_specific_target(
    ...
    raise NotImplementedError(
        f"macOS version {'.'.join(str(v) for v in version)} is not supported"
    )
